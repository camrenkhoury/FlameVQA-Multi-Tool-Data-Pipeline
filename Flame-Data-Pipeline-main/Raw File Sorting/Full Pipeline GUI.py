...
class PairPreviewWindow:
    def __init__(
        ...
        self.calibration_tool_button.grid(row=0, column=3, sticky="ew", padx=8)
        self.transform_selector_button = ttk.Button(
            actions,
            text="Dataset Transform Selector",
            command=self._open_transform_selector,
        )
        self.transform_selector_button.grid(row=0, column=4, sticky="ew", padx=(8, 0))
        ...
        ).grid(row=1, column=1, columnspan=4, sticky="ew", pady=(8, 0))
    ...
    def _update_metrics_text(self, debug_info):
        ...
        fire_alignment = alignment.get("fire_alignment", {})
        ...
        self.metrics_var.set(
            ...
            f"fire_overlap={alignment.get('fire_overlap_status', '')} "
            f"{alignment.get('fire_overlap_before', '')}->{alignment.get('fire_overlap_after', '')} | "
            f"fire_shift=({alignment.get('fire_alignment_shift_x', 0.0):.1f}, {alignment.get('fire_alignment_shift_y', 0.0):.1f}) | "
            f"fire_source_coverage={fire_alignment.get('source_coverage_before', '')}->{fire_alignment.get('source_coverage_after', '')} | "
    ...
    def _open_transform_selector(self):
        ConsensusTransformSelectionWindow(self.window, self.sorter, self.burn_set)
...
class ConsensusTransformSelectionWindow:
    def __init__(
        self,
        parent,
        sorter_module,
        ...
        self.sorter = sorter_module
        self.burn_set = burn_set
        self.pairs = list(burn_set.get("pairs", []))
        self.on_profile_selected = on_profile_selected
        self.on_cancel = on_cancel
        self.progress_callback = progress_callback
        self.apply_to_output = apply_to_output
        self.dataset_name = burn_set.get("dataset_name") or "dataset"
        self.burn_set_name = burn_set.get("name") or burn_set.get("burn_set_name") or "burn_set_001"
        self.detected_camera = burn_set.get("detected_camera", self.sorter.CAMERA_USED)
        self.attempt_index = 0
        self.sample_seed = 1337
        self.candidates = []
        self.preview_images = []
        self.selection_vars = {}
        ...
        self.window.title(f"Dataset Transform Selector - {self.dataset_name} / {self.burn_set_name}")
        self.window.geometry("1680x980")
        self.window.minsize(1250, 760)
        ...
        ttk.Label(
            outer,
            text=f"Dataset Transform Selector: {self.dataset_name} / {self.burn_set_name}",
            font=("Segoe UI", 13, "bold"),
        ).grid(row=0, column=0, sticky="w")
        ttk.Label(
            outer,
            text=(
                "Review representative AUTO_ALIGN candidates, select the best three, "
                "then apply their fitted consensus transform to every Corrected FOV image in this output folder."
            ),
            wraplength=1300,
        ...
        self.rows_frame.bind("<MouseWheel>", self._handle_mousewheel)
        ...
        self.retry_button = ttk.Button(
            controls,
            text="Try Different Candidate Set",
            command=self._retry_candidates,
        )
        self.retry_button.grid(row=0, column=1, sticky="e", padx=(8, 0))
        self.apply_button = ttk.Button(
            controls,
            text="Apply Selected Transform" if self.apply_to_output else "Use Selected Transform For Full Export",
            command=self._apply_selected,
            state="disabled",
        )
        self.apply_button.grid(row=0, column=2, sticky="e", padx=(8, 0))

        self._start_candidate_generation()

    def _show_window_after_generation(self):
    ...
    def _burn_output_root(self):
        images_root = Path(self.burn_set["source_root"])
        return images_root.parent if images_root.name.lower() == "images" else images_root

    def _images_root(self):
        images_root = Path(self.burn_set["source_root"])
        return images_root if images_root.name.lower() == "images" else images_root / "Images"

    def _corrected_output_dir(self):
    ...
    def _candidate_pair_for_mode(self, pair, source_mode):
        candidate_pair = dict(pair)
        if source_mode == "thermal_jpg_only" and pair.get("thermal_jpg") is not None:
            candidate_pair["cal_tiff"] = None
            candidate_pair["thermal_tiff"] = None
        elif source_mode == "thermal_tiff_only" and pair.get("thermal_tiff") is not None:
            candidate_pair["cal_tiff"] = None
    ...
    def _generate_candidate_corrected_fov(self, candidate_pair, pair, source_mode):
        ...
        try:
            if source_mode == "similarity_only":
                self.sorter.FEATURE_ALIGNMENT_MODEL = "similarity"
            elif source_mode == "affine_only":
                self.sorter.FEATURE_ALIGNMENT_MODEL = "affine"
            return self.sorter.generate_corrected_fov(
                candidate_pair,
                mode="AUTO_ALIGN",
                dataset_name=pair.get("dataset_name", self.dataset_name),
                burn_set_name=pair.get("burn_set_name", self.burn_set_name),
            ...
            self.sorter.FEATURE_ALIGNMENT_MODEL = original_model
    ...
    def _load_display_image(self, path):
        if not path or not Path(path).exists():
            return Image.new("RGB", self.sorter._get_output_size(), color=(40, 40, 40))
        image = Image.open(path)
        if image.mode not in ("RGB", "RGBA"):
            image = ImageOps.autocontrast(image.convert("L")).convert("RGB")
        else:
    ...
    def _fit_panel(self, image, panel_size):
        panel = Image.new("RGB", panel_size, color=(245, 245, 245))
        fitted = image.copy()
        fitted.thumbnail(panel_size)
        panel.paste(fitted, ((panel_size[0] - fitted.width) // 2, (panel_size[1] - fitted.height) // 2))
        return panel

    def _blend_overlay(self, corrected_image, thermal_image):
        thermal = thermal_image.convert("RGB").resize(corrected_image.size, Image.LANCZOS)
        return Image.blend(corrected_image.convert("RGB"), thermal, 0.50)

    def _compose_candidate_preview(self, corrected_image, thermal_image, pair_index, alignment, source_mode):
        panel_size = (420, 336)
        label_height = 68
        gap = 16
        overlay = self._blend_overlay(corrected_image, thermal_image)
        items = [
            ("Candidate Corrected FOV", corrected_image),
            ("Thermal Preview", thermal_image),
            ("Candidate vs Thermal", overlay),
        ]
        width = len(items) * panel_size[0] + (len(items) + 1) * gap
        height = panel_size[1] + label_height + gap * 2
        canvas = Image.new("RGB", (width, height), color=(255, 255, 255))
        draw = ImageDraw.Draw(canvas)
        for column, (label, image) in enumerate(items):
            x = gap + column * (panel_size[0] + gap)
            draw.text((x, gap), label, fill=(0, 0, 0))
            canvas.paste(self._fit_panel(image, panel_size), (x, gap + label_height))

        metric = (
            f"sample={pair_index + 1} source={source_mode} "
            f"conf={alignment.get('confidence_level', '')} "
            f"inliers={alignment.get('inliers', 0)} "
            f"scale=({alignment.get('scale_x', 1.0):.4f}, {alignment.get('scale_y', 1.0):.4f}) "
            f"tx={alignment.get('translation_x', 0.0):.1f} ty={alignment.get('translation_y', 0.0):.1f} "
            f"fire={alignment.get('fire_overlap_status', '')} "
            f"{alignment.get('fire_overlap_before', '')}->{alignment.get('fire_overlap_after', '')} "
            f"coverage={alignment.get('fire_alignment', {}).get('source_coverage_before', '')}->{alignment.get('fire_alignment', {}).get('source_coverage_after', '')}"
        )
        draw.text((gap, 28), metric, fill=(40, 40, 40))
        overlay.close()
        return canvas

    def _start_candidate_generation(self):
        ...
        if not self.pairs:
            self.status_var.set("No pairs are available for this burn set.")
            self._show_window_after_generation()
            return

        source_mode = self._candidate_source_mode()
        if self.progress_callback:
            self.progress_callback(
                {
                    "current": 0,
                    "total": max(1, min(len(self.pairs), self.sorter.CONSENSUS_ALIGNMENT_SAMPLE_TARGET)),
                    "message": f"generating samples for {self.dataset_name} / {self.burn_set_name}",
                }
            )
        self.status_var.set(
            f"Generating representative candidates with source mode '{source_mode}'..."
        )
        threading.Thread(
            target=self._generate_candidates_worker,
            args=(source_mode,),
            daemon=True,
        ).start()

    def _generate_candidates_worker(self, source_mode):
        try:
            indices = self.sorter.representative_transform_sample_indices(
                len(self.pairs),
                seed=self.sample_seed + self.attempt_index * 1009,
            )
            candidates = []
            for sample_number, pair_index in enumerate(indices, start=1):
                pair = self.pairs[pair_index]
                candidate_pair = self._candidate_pair_for_mode(pair, source_mode)
                corrected = None
                thermal = None
                try:
                    corrected, debug_info = self._generate_candidate_corrected_fov(
                        candidate_pair,
                        pair,
                        source_mode,
                    )
                    if corrected.mode not in ("RGB", "RGBA"):
                        ...
                        corrected = corrected.convert("RGB")
                    alignment = debug_info.get("feature_alignment", {})
                    thermal = self._load_display_image(self._thermal_preview_source_path(pair))
                    preview = self._compose_candidate_preview(
                        corrected,
                        thermal,
                        pair_index,
                        alignment,
                    ...
                    selectable = bool(self.sorter.alignment_has_selectable_transform(alignment))
                    if selectable and not alignment.get("accepted"):
                        selection_note = "Selectable by visual review despite LOW automatic confidence"
                    elif selectable:
                        selection_note = "Selectable"
                    else:
                        selection_note = "Not selectable: no usable transform"
                    candidates.append(
                        {
                            "id": f"{source_mode}_{pair_index}",
                            "sample_number": sample_number,
                            "pair_index": pair_index,
                            "rgb_filename": Path(pair["rgb"]["filename"]).name,
                            "source_mode": source_mode,
                            "alignment": alignment,
                            "selectable": selectable,
                            "selection_note": selection_note,
                            "preview": preview,
                            "reasons": " | ".join(alignment.get("reasons", [])),
                        }
                    )
                except Exception as exc:
                    candidates.append(
                        {
                            ...
                            "alignment": {},
                            "selectable": False,
                            "selection_note": "Not selectable: candidate failed",
                            "preview": None,
                            "reasons": f"candidate_failed:{exc}",
                        }
                    )
                finally:
                    ...
                    self.progress_callback(
                        {
                            ...
                            "total": len(indices),
                            "message": f"{self.dataset_name} / {self.burn_set_name}: sample {sample_number}/{len(indices)}",
                        }
                    )
            ...
            self.window.after(0, self._candidate_generation_failed, error_text)

    def _candidate_generation_failed(self, error_text):
        self.retry_button.configure(state="normal")
        self.status_var.set("Candidate generation failed.")
        self._show_window_after_generation()
        messagebox.showerror("Dataset Transform Selector", error_text)

    def _populate_candidates(self, candidates, source_mode):
        # Keep all candidates on the class for diagnostic CSV output, but only
        # render the selectable ones. Non-selectable rows (no usable transform,
        # candidate-generation failure, or fire-gate rejection) are noise in
        # the picker because the user can't act on them.
        self.candidates = candidates
        selectable_candidates = [c for c in candidates if c.get("selectable")]
        hidden_count = len(candidates) - len(selectable_candidates)
        selectable_count = len(selectable_candidates)
        required = min(3, len(self.pairs))
        if selectable_count == 0:
            status_text = (
                f"Generated {len(candidates)} candidates using '{source_mode}' but "
                f"zero are selectable (all rejected by fire gate or alignment). "
                f"Click 'Try Different Candidate Set' to resample."
            )
        elif selectable_count < required:
            status_text = (
        ...
        if self.progress_callback:
            self.progress_callback(
                {
                    "current": max(1, len(candidates)),
                    "total": max(1, len(candidates)),
                    "message": f"{self.dataset_name} / {self.burn_set_name}: ready for review",
                }
            )

        for row_index, candidate in enumerate(selectable_candidates):
            row = ttk.Frame(self.rows_frame, padding=(4, 14))
            row.grid(row=row_index, column=0, sticky="ew")
            row.columnconfigure(1, weight=1)

            var = tk.BooleanVar(value=False)
            self.selection_vars[candidate["id"]] = var
            checkbox = ttk.Checkbutton(row, variable=var)
            checkbox.grid(row=0, column=0, rowspan=2, sticky="n", padx=(0, 10))

            if candidate["preview"] is not None:
                preview = candidate["preview"].copy()
                preview.thumbnail((1360, 470))
                photo = ImageTk.PhotoImage(preview)
                self.preview_images.append(photo)
                ttk.Label(row, image=photo).grid(row=0, column=1, sticky="w")
            else:
                ttk.Label(row, text="Preview unavailable").grid(row=0, column=1, sticky="w")

            alignment = candidate["alignment"]
            detail = (
                f"{candidate['sample_number']:02d}. pair {candidate['pair_index'] + 1} | "
                f"{candidate['rgb_filename']} | "
                f"status={alignment.get('status', 'failed')} | "
                f"confidence={alignment.get('confidence_level', '')} | "
                f"model={alignment.get('transform_type', '')} | "
                f"prep={alignment.get('representation', '')} | "
                f"RMSE={alignment.get('mean_reprojection_error_px', '')} | "
                f"fire={alignment.get('fire_overlap_status', '')} "
                f"{alignment.get('fire_overlap_before', '')}->{alignment.get('fire_overlap_after', '')} "
                f"shift=({alignment.get('fire_alignment_shift_x', 0.0):.1f}, {alignment.get('fire_alignment_shift_y', 0.0):.1f}) | "
                f"coverage={alignment.get('fire_alignment', {}).get('source_coverage_before', '')}->{alignment.get('fire_alignment', {}).get('source_coverage_after', '')} | "
                f"{candidate.get('selection_note', '')} | "
                f"reasons={candidate['reasons'] or 'none'}"
            )
            ttk.Label(row, text=detail, wraplength=1320, justify="left").grid(
                row=1,
                column=1,
                sticky="ew",
                pady=(8, 0),
    ...
    def _selected_candidates(self):
        ...
        for candidate in self.candidates:
            var = self.selection_vars.get(candidate["id"])
    ...
    def _apply_selected(self):
        ...
        self.retry_button.configure(state="disabled")
        status_text = (
            "Applying fitted consensus transform to every Corrected FOV image..."
            if self.apply_to_output
            else "Saving fitted consensus transform for the full export..."
        )
        self.status_var.set(status_text)
        threading.Thread(
            target=self._apply_selected_worker,
            args=(selected,),
            daemon=True,
        ).start()

    def _apply_selected_worker(self, selected):
        try:
            transform_matrix = self.sorter.consensus_transform_matrix_from_alignments(
                [candidate["alignment"] for candidate in selected]
            )
            output_root = self._burn_output_root()
            profile_path = output_root / self.sorter.CONSENSUS_ALIGNMENT_PROFILE_FILENAME
            profile = {
                "version": 1,
                "created_at": datetime.datetime.utcnow().replace(microsecond=0).isoformat() + "Z",
                "dataset_name": self.dataset_name,
                "burn_set_name": self.burn_set_name,
                "source_root": str(self.burn_set.get("source_root", "")),
                "sample_seed": self.sample_seed,
                "attempt_index": self.attempt_index,
                "attempt_source_mode": self._candidate_source_mode(),
                "sample_count": len(self.candidates),
                "selected_count": len(selected),
                "consensus_method": "robust_grid_median_affine_best_fit",
                "consensus_transform_matrix": transform_matrix,
                "candidate_samples": [
                    {
                        "pair_index": candidate["pair_index"] + 1,
                        "rgb_filename": candidate["rgb_filename"],
            ...
            if self.on_profile_selected is not None:
                self.window.after(0, self._profile_selected, profile)
                return
            self.sorter.save_consensus_alignment_profile(str(profile_path), profile)

            def progress_callback(progress):
                self.window.after(0, self.status_var.set, progress["message"])

            count = self.sorter.apply_consensus_transform_to_pairs(
                self.pairs,
                transform_matrix,
                str(self._corrected_output_dir()),
                dataset_name=self.dataset_name,
                burn_set_name=self.burn_set_name,
                camera_used=self.detected_camera,
    ...
    def _profile_selected(self, profile):
        self.retry_button.configure(state="normal")
    ...
    def _apply_complete(self, count, profile_path):
        self.retry_button.configure(state="normal")
        self.apply_button.configure(state="normal")
        self.status_var.set(f"Applied consensus transform to {count} image(s). Profile saved: {profile_path}")
        messagebox.showinfo(
            "Dataset Transform Selector",
            f"Applied the selected fitted consensus transform to {count} Corrected FOV image(s).\n\nProfile saved:\n{profile_path}",
        )

    def _apply_failed(self, error_text):
        self.retry_button.configure(state="normal")
        self.apply_button.configure(state="normal")
        self.status_var.set("Consensus transform application failed.")
        messagebox.showerror("Dataset Transform Selector", error_text)
...
class FullPipelineGui:
    def __init__(self, root):
        ...
        self.status_var = tk.StringVar(
            value="Set folders, then run the full pipeline. Manual transform review runs before full export."
        )
        self.alignment_progress_var = tk.DoubleVar(value=0.0)
        self.alignment_progress_label_var = tk.StringVar(value="0. Manual transform evaluation: idle")
        ...
        self.manual_alignment_var = tk.BooleanVar(value=True)
        ...
        self.pending_sort_input_folder = None
    ...
    def _build_ui(self):
        ...
        ttk.Label(
            ...
            text=(
                ...
                "Manual transform review can run first; the selected automation steps run after export completes."
            ),
            wraplength=1280,
        ...
        automation = ttk.LabelFrame(controls, text="Pipeline Options", padding=8)
        automation.grid(row=2, column=0, columnspan=8, sticky="ew", pady=(12, 0))
        automation.columnconfigure(5, weight=1)
        ttk.Checkbutton(
            automation,
            text="Manual transform review before export",
            variable=self.manual_alignment_var,
        ...
        ttk.Label(pipeline_progress, textvariable=self.alignment_progress_label_var, width=52).grid(
            row=0, column=0, sticky="w", padx=(0, 8)
        )
        self.alignment_progress_bar = ttk.Progressbar(
            pipeline_progress,
            variable=self.alignment_progress_var,
            maximum=100,
            mode="determinate",
        )
        self.alignment_progress_bar.grid(row=0, column=1, sticky="ew")
        ...
        self.progress_bar.grid(row=1, column=1, sticky="ew", pady=(6, 0))
        ...
        self.gps_progress_bar.grid(row=2, column=1, sticky="ew", pady=(6, 0))
        ...
        self.labeling_progress_bar.grid(row=3, column=1, sticky="ew", pady=(6, 0))
    ...
    def _reset_results_state(self):
        ...
        self.alignment_progress_label_var.set("0. Manual transform evaluation: idle")
    ...
    def _poll_output_queue(self):
        try:
            while True:
                ...
                elif kind == "alignment_progress":
                    current = payload["current"]
                    total = payload["total"] if payload["total"] else 1
                    percent = (current / total) * 100.0
                    self.alignment_progress_var.set(percent)
                    self.alignment_progress_label_var.set(
                        f"0. Manual transform evaluation: {current}/{total} - {payload['message']}"
                    )
                elif kind == "alignment_analysis_ready":
                    self._handle_alignment_analysis_ready(payload)
                elif kind == "alignment_complete":
                    self.alignment_progress_var.set(100.0)
                    self.alignment_progress_label_var.set("0. Manual transform evaluation: complete")
                    self._start_sort_worker(
                        self.pending_sort_input_folder,
                        self.pending_sort_output_folder,
                        consensus_alignment_profiles=self.consensus_alignment_profiles,
                    )
                elif kind == "alignment_skipped":
                    self.alignment_progress_var.set(100.0)
                    self.alignment_progress_label_var.set(f"0. Manual transform evaluation: skipped - {payload}")
                    self._start_sort_worker(
                        self.pending_sort_input_folder,
                        self.pending_sort_output_folder,
    ...
    def _run_sort(self):
        ...
        self.pending_sort_input_folder = input_folder
        self.pending_sort_output_folder = output_folder
        self.consensus_alignment_profiles = {}
        if self.manual_alignment_var.get():
            self.alignment_progress_var.set(0)
            self.alignment_progress_label_var.set("0. Manual transform evaluation: pairing input")
            self.progress_label_var.set("1. Raw sorting/export: waiting for manual transform evaluation")
            self._start_manual_alignment_preflight(input_folder, output_folder)
        else:
            self.alignment_progress_var.set(100.0)
            self.alignment_progress_label_var.set("0. Manual transform evaluation: skipped (disabled)")
            self._start_sort_worker(input_folder, output_folder)

    def _start_sort_worker(self, input_folder, output_folder, consensus_alignment_profiles=None):
        self.status_var.set("Running raw sorting/export...")
        self.progress_var.set(0)
        self.progress_label_var.set("1. Raw sorting/export: preparing")
        ...
        def worker():
            ...
            try:
                ...
                # Show the user the camera-validation reason directly instead
                # of a noisy Python traceback - this is the mixed/unknown-camera
                # halt and it's a normal failure mode, not a crash.
                self.output_queue.put(
                    ("error", f"Camera validation halted the pipeline.\n\n{exc}")
                )
                return
            except Exception:
    ...
    def _start_manual_alignment_preflight(self, input_folder, output_folder):
        def worker():
            try:
                selected_mode = self.mode_var.get()
                if selected_mode == "AUTO":
                    selected_mode = sorter.detect_processing_mode(input_folder)
                if selected_mode != "PRESORTED_STANDARD":
                    self.output_queue.put((
                        "alignment_skipped",
                        f"{selected_mode} input cannot be sampled before export",
                    ))
                    return
                ...
                burn_sets = [
                    ...
                    if burn_set.get("pairs")
                ]
                if not burn_sets:
                    self.output_queue.put(("alignment_skipped", "no matched pairs found"))
                    return
                self.output_queue.put((
                    "alignment_analysis_ready",
                    {
                        "analysis": analysis,
                        "burn_sets": burn_sets,
    ...
    def _handle_alignment_analysis_ready(self, payload):
    ...
    def _open_next_manual_alignment_selector(self):
        if self.pending_alignment_index >= len(self.pending_alignment_burn_sets):
            self.output_queue.put(("alignment_complete", None))
            return
    ...
    def _manual_alignment_profile_selected(self, profile):
        ...
        self.consensus_alignment_profiles[key] = profile
        self.pending_alignment_index += 1
        completed = self.pending_alignment_index
